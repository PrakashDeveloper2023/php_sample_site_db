...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id'])){
    ...
    $master = new Master();
    $data = $master->get_content_details($_POST['id']);
    if(!$data){
...
<div class="container-fluid">
    <form action="" method="POST" id="manage_form">
        ...
        <div class="mb-3">
            ...
            <textarea name="content" id="content" cols="30" rows="10" class="form-control rounded-0"><?= isset($data['content']) ? $data['content'] : "" ?></textarea>
        </div>
    </form>
</div>
...
<script>
    $(function(){
        ...
        $('#manage_form').submit(function(e){
            ...
            tinyMCE.triggerSave();
            $.ajax({
                url: "ajax-api.php?action=save_content",
                method: "POST",
                ...
                error: err=> {
                    ...
                    _this.prepend(el)
                    el.show("toggle")
                    $('#FormModal .modal-body').scrollTop(0)
                    end_loader()
                    console.error(err)
                },
